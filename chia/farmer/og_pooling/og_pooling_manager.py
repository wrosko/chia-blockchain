...
class OgPoolingManager:
    ...
    _update_pool_info_task: Task | None = None
    _update_pool_difficulty_task: Task | None = None
    ...
    def __init__(
        ...
        self.update_pool_payout_address(
            farmer_config=farmer_config, farmer_reward_target_address=farmer_reward_target_address
        )
        ...
        self._pool_and_farmer_private_keys_map = {
            bytes(key.get_g1()): key for key in self._pool_and_farmer_private_keys
        }
        self._pool_public_keys = pool_public_keys

    async def initialize_pooling(self):
        if self._is_og_pooling_disabled:
            self._logger.info("Not OG pooling as 'disable_og_pooling' is set to true in your config")
        ...
        while not is_connected_to_og_pool:
            try:
                ...
                self._logger.info(
                    f"Connected to OG pool {pool_name} ({self._pool_url}) "
                    f"using payout address {self._pool_payout_address}"
                )
                is_connected_to_og_pool = True
            except asyncio.TimeoutError:
                self._logger.error("Timed out while retrieving OG pool info")
                await sleep(5)
    ...
    async def process_new_proof_of_space_for_og_pool(
        ...
        try:
            ...
            self._logger.error("Timed out while submitting partial to OG pool")
            return
        except Exception as e:
        ...
        if "error_code" in submit_partial_response:
            if submit_partial_response["error_code"] == 5:
                ...
                self._logger.error(
                    f"Error in OG pooling: "
                    f"{submit_partial_response['error_code'], submit_partial_response['error_message']}"
                )
        else:
    ...
    async def _update_pool_info(self, pool_info: dict[str, Any] | None = None):
        ...
        if did_request_new_pool_info:
            self._logger.info("Updated the OG pool_info successfully")

    async def _update_pool_difficulty(self):
        difficulties_with_none = await gather(
            *[self._get_og_farmer_difficulty(pool_public_key) for pool_public_key in self._pool_public_keys]
        )
    ...
    async def _get_og_farmer_difficulty(self, pool_public_key: G1Element) -> uint64 | None:
